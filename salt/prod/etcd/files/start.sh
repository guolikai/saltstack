...
fpid()
 {
    AppMasterPid=$(ps ax | grep  "etcd" | grep -v "grep" | awk '{print $1}' 2> /dev/null)
#    echo $AppMasterPid
}
...
fcpconf()
{
    cp $AppInstallDir{etcd,etcdctl}  /usr/sbin/
...
fupdate()
{
    ...
    cp -rf $AppBuildDir $AppInstallDir 
...
#        /usr/bin/nohup /App/opt/OPS/etcd/etcd --name auto_scale --data-dir /App/data/  --listen-peer-urls "http://10.10.10.10:2380,http://10.10.10.10:7001" --listen-client-urls "http://10.10.10.10:2379,http://10.10.10.10:4001" --advertise-client-urls "http://10.10.10.10:2379,http://10.10.10.10:4001" &		
